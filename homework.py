...
class SportsWalking(Training):
    ...
    CALORIES_MEAN_SPEED_MULTI_SECOND = 0.029
    SHIFT_SPEED = 0.278
    CM_IN_M = 100
    ...
    def get_spent_calories(self) -> float:
        """Получить количество затраченных калорий."""
        return ((self.CALORIES_MEAN_SPEED_MULTI * self.weight
                 + (((self.get_mean_speed() * self.SHIFT_SPEED)**2)
                     / (self.height / self.CM_IN_M))
                 * self.CALORIES_MEAN_SPEED_MULTI_SECOND
